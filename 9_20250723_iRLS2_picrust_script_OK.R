...
#KO_Differential Analysis####
rm(list=ls())
setwd("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/")
KO_AbsAbundance <- read.table("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/picrust2_output/KO_metagenome_out/pred_metagenome_unstrat.tsv", header=T,sep="\t")
rownames(KO_AbsAbundance) <- KO_AbsAbundance$function.
KO_AbsAbundance <- KO_AbsAbundance[,-1]
KO_AbsAbundance <- droplevels(KO_AbsAbundance)
head(KO_AbsAbundance)

KO_RelAbundance <-KO_AbsAbundance
KO_RelAbundance<-clr(KO_RelAbundance+1)

#for (i in 1:ncol(KO_AbsAbundance)){
#  KO_RelAbundance[,i] <- KO_AbsAbundance[,i]/sum(KO_AbsAbundance[,i],na.rm=T)
#}

##Samplesheet and Sample Filter####
ss <- read.table("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/iRLS2_Metagenomics_Samplesheet_picrust.txt", sep="\t", header=T)
ss_filt <- ss[ss$Group3%in%c("CTRL","iRLS","insonne"),]
ss_filt <- droplevels(ss_filt)
dim(ss_filt)

KO_RelAbundance_filt <- KO_RelAbundance[,colnames(KO_RelAbundance)%in%ss_filt$SampleID]
dim(KO_RelAbundance_filt)

ss_filt <- ss_filt[ss_filt$SampleID%in%colnames(KO_RelAbundance_filt),]
dim(ss_filt)

table(ss_filt$SampleID==colnames(KO_RelAbundance_filt)) #sono in ordine
...
design <- model.matrix(~0+Group+Sesso+Age+BMI+Psichiatrico, data=KO_RelAbundance_filt)
head(design)
fit <- lmFit(KO_RelAbundance_filt,design=design)
#iRLS vs Ctrl
...
topl1_sig <- topl1[topl1$P.Value<0.05&!is.na(topl1$P.Value),]
write.table(topl1,file="20250723_iRLSvCTR_Limma_KO.txt", sep="\t")
...
topl2_sig <- topl2[topl2$P.Value<0.05&!is.na(topl2$P.Value),]
write.table(topl2,file="20250723_iRLSvsinsonne_Limma_KO.txt", sep="\t")
...
topl3_sig <- topl3[topl3$P.Value<0.05&!is.na(topl3$P.Value),]
write.table(topl3,file="20250723_insonnevsCTRL_Limma_KO.txt", sep="\t")

#pathway_Differential Analysis####
rm(list=ls())
setwd("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/")
p_AbsAbundance <- read.table("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/picrust2_output/pathways_out/path_abun_unstrat.tsv", header=T,sep="\t")
rownames(p_AbsAbundance) <- p_AbsAbundance$pathway
p_AbsAbundance <- p_AbsAbundance[,-1]
p_AbsAbundance <- droplevels(p_AbsAbundance)
head(p_AbsAbundance)

p_RelAbundance <-p_AbsAbundance
...
design <- model.matrix(~0+Group+Sesso+Age+BMI+Psichiatrico, data=p_RelAbundance_filt)
head(design)
...
topl1 <- data.frame(rownames(topl1),topl1)
topl1_sig <- topl1[topl1$P.Value<0.05&!is.na(topl1$P.Value),]
write.table(topl1,file="20250723_iRLSvsCTR_Limma_pathway.txt", sep="\t")
...
topl2 <- data.frame(rownames(topl2),topl2)
topl2_sig <- topl2[topl2$P.Value<0.05&!is.na(topl2$P.Value),]
write.table(topl2,file="20250723_iRLSvsinsonne_Limma_pathway.txt", sep="\t")
...
topl3 <- data.frame(rownames(topl3),topl3)
topl3_sig <- topl3[topl3$P.Value<0.05&!is.na(topl3$P.Value),]
write.table(topl3,file="20250723_insonnevsCTRL_Limma_pathway.txt", sep="\t")

#EC_Differential Analysis####
rm(list=ls())
setwd("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/")
EC_AbsAbundance <- read.table("/standard/users/m.bacalini/METAGENOMICS/IRLS2_picrust2/picrust2_output/EC_metagenome_out/pred_metagenome_unstrat.tsv", header=T,sep="\t")
...
EC_RelAbundance <-EC_AbsAbundance
...
design <- model.matrix(~0+Group+Sesso+Age+BMI+Psichiatrico, data=EC_RelAbundance_filt)
head(design)
...
topl1 <- data.frame(rownames(topl1),topl1)
topl1_sig <- topl1[topl1$P.Value<0.05&!is.na(topl1$P.Value),]
write.table(topl1,file="20250723_iRLSvsCTR_Limma_EC.txt", sep="\t")
...
topl2 <- data.frame(rownames(topl2),topl2)
topl2_sig <- topl2[topl2$P.Value<0.05&!is.na(topl2$P.Value),]
write.table(topl2,file="20250723_iRLSvsinsonne_Limma_EC.txt", sep="\t")
...
topl3 <- data.frame(rownames(topl3),topl3)
topl3_sig <- topl3[topl3$P.Value<0.05&!is.na(topl3$P.Value),]
write.table(topl3,file="20250723_insonnevsCTRL_Limma_EC.txt", sep="\t")
